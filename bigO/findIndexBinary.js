...
const findIndex = (arr, num) => {
  if(arr[arr.length-1]===num)return `found at ${arr.length-1}`
  let arrayStart = 0
  let arrayEnd = arr.length-1
  let middle = Math.floor(arr.length/2)
  while(true){
    if(arr[middle]===num)return `found at ${middle}`
    else if(arr[middle] < num){
      middle += Math.floor((arrayEnd-middle)/2)
      arrayStart = middle
    }else{
      arrayEnd = middle
      middle = Math.floor((middle-arrayStart)/2)
    }
  }
}
